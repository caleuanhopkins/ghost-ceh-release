...
coreHelpers = function (ghost) {
    ...
    // `{{postImage}}`
    //
    // Make the Post Image available outside the {{#post}} scope. Useful for social media meta tags in header.
    //
    //
    // **returns** SafeString URL of post image set in Ghost Editor view.
    //
    ghost.registerThemeHelper('postImage', function (context, options) {
        var  image, blog;
        ...
        return ghost.doFilter('postImage', image, function (title) {
            return new hbs.handlebars.SafeString(image.trim());
        });
    });
    ...
    // i.e. If inside a post context will return post permalink
    // absolute flag outputs absolute URL, else URL is relative
    ghost.registerThemeHelper('theURL', function (options) {
        var theURL, blog;
        blog = ghost.blogGlobals();
        if (_.isString(this.path)) {
            if (!this.path || this.path === '/' || this.path === '' || this.path.match(/\/page/)) {
                blog = ghost.blogGlobals();
                theURL = ghost.config().url;
            } else {
                theURL = ghost.config().url+'/'+this.post.slug;
            }
        }
        return ghost.doFilter('theURL', theURL, function (title) {
            return new hbs.handlebars.SafeString(theURL.trim());
        });
    });
